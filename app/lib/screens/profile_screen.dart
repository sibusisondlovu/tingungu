          ...
          return ListView(
            ...
            children: [
              ...
              const Padding(
                ...
                child: Text('Transaction History', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF3B0D11))),
              ),
              StreamBuilder<QuerySnapshot>(
                stream: FirebaseFirestore.instance.collection('users').doc(user?.uid).collection('transactions').orderBy('date', descending: true).limit(10).snapshots(),
                builder: (context, txSnapshot) {
                  if (!txSnapshot.hasData) return const Center(child: CircularProgressIndicator());
                  final txs = txSnapshot.data!.docs;
                  if (txs.isEmpty) return const Text('No transactions yet.', style: TextStyle(color: Colors.grey));
                  ...
                  return Column(
                    children: txs.map((tx) {
                      ...
                      return Card(
                        ...
                        child: ListTile(
                          leading: const Icon(Icons.receipt_long, color: Color(0xFFFB8B24)),
                          title: Text(type),
                          subtitle: Text(data['date'] != null ? (data['date'] as Timestamp).toDate().toString().split('.')[0] : 'Pending'),
                          trailing: Text('R $amount', style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF3B0D11))),
                        ),
                      );
                    }).toList(),
                  );
